...
describe('FeedbackForm', () => {
  it('renders email and message fields', () => {
    ...
    expect(screen.getByLabelText('Sähköposti (vapaaehtoinen)')).toBeInTheDocument();
  ...
  it('shows a validation error and does not submit when the message is too short', async () => {
    ...
    expect(
      await screen.findByText('Täytä pakolliset kentät ja kerro hieman tarkemmin.'),
    ).toBeInTheDocument();
  ...
  it('submits successfully and shows a success message', async () => {
    ...
    expect(await screen.findByText('Palaute lähetetty!')).toBeInTheDocument();
  });
  ...
  it('treats honeypot submissions as success without calling the server action', async () => {
    ...
    await userEvent.type(messageInput, 'Tämä palaute tulisi ohittaa honeypotin ansiosta.');
    await userEvent.click(submitButton);
