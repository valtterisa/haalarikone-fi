...
export function SearchModal({ triggerLabel, placeholder, modalTitle }: SearchModalProps) {
  ...
  const groupedByColor = useMemo(() => {
    ...
    results.forEach((uni) => {
      ...
      if (!groups.has(primaryColorBase)) {
        groups.set(primaryColorBase, { unis: [], hex: null });
      }
      const group = groups.get(primaryColorBase)!;
      group.unis.push(uni);
                          ...
                          .map(([color, data]) => (
                            ...
                            >
                              <div
                                ...
                                style={
                                  data.hex ? parseStyles(data.hex) : { backgroundColor: '#e5e7eb' }
                                }
                              />
                              <div className="flex-1 min-w-0">
